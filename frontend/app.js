...
const savedTheme = localStorage.getItem('kelper-theme') || 'light';
...
function saveFormData() {
  const data = {};
  document.querySelectorAll('#customizer-form input').forEach(el => { data[el.id] = el.value; });
  localStorage.setItem('kelper-form-data', JSON.stringify(data));
}

function loadFormData() {
  const saved = localStorage.getItem('kelper-form-data');
  if (!saved) return;
  try {
...
function populateVersions() {
  const keys = Object.keys(localStorage).filter(k => k.startsWith('kelper-version-'));
  loadVersionSelect.innerHTML = '<option value="">Load Version...</option>';
  keys.forEach(k => {
    const opt = document.createElement('option');
    opt.value = k;
    opt.textContent = k.replace('kelper-version-', '');
    loadVersionSelect.appendChild(opt);
  });
}

saveVersionBtn.addEventListener('click', () => {
  const name = prompt('Enter version name:', `v${Object.keys(localStorage).filter(k => k.startsWith('kelper-version-')).length + 1}`);
  if (!name) return;
  const data = {};
  document.querySelectorAll('#customizer-form input').forEach(el => { data[el.id] = el.value; });
  localStorage.setItem(`kelper-version-${name}`, JSON.stringify({ tab: activeTab, data, html: contractView.innerHTML }));
  populateVersions();
...
if (emailBtn) {
  emailBtn.addEventListener('click', () => {
    ...
    window.location.href = `mailto:?subject=Kelper Document&body=${text}`;
...
downloadMdBtn.addEventListener('click', () => {
  ...
  md += `*Generated by Kelper on ${new Date().toLocaleDateString()}*\n`;
...
if (!localStorage.getItem('kelper-wizard-seen')) {
  openWizard();
  localStorage.setItem('kelper-wizard-seen', '1');
}
